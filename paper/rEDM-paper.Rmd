...
Since the time series come from a discrete map, a time lag of `tau = 1` is appropriate (and also the default for functions in the **rEDM** package). Because the dynamics are fairly simple, we can also use the default range for the embedding dimension, $E = 1:10$. (A more general range to test would be up to $sqrt(n)$ with $n$ being the length of the time series [REF]).
...
Here, the `simplex()` function applies Simplex Projection, a method that infers the dynamical model $F^\prime$ using nearest neighbors [@Sugihara_1990]. 
