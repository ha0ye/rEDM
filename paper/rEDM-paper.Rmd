...
The essential concept underlying EDM is that time series can be viewed as projections of the behavior of a dynamic system. Here, we assume that the state of the dynamic system can be represented as a point in a high-dimensional space. Typically, the different axes of this space are the fundamental state variables. In an ecosystem, these variables could be population abundances, resources, or environmental conditions. We also assume that the system changes over time following a set of deterministic rules. In other words, the behavior of the system is not completely stochastic.

Given this description of a dynamic system, it is possible to construct a time series by projecting the system state onto one of the coordinate axes -- sequential observations of that variable thus produce a time series for that variable. For example, in Figure \@ref(fig:time-series-projection), the states of the canonical Lorenz Attractor [-@Lorenz_1963] are projected to the $x$-axis, creating a time series of variable $x$. In simple cases, each time series corresponds to a different state variable. However, more generally, a time series is an *observation function* of the system state, and may be a function of one or several state variables.
