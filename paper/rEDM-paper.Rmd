...
abstract: "
  1. Modeling the ecological time series can be challenging because of uncertainty about the mathematical representation behind the underlying processes, or because of dependence on unknown or unobserved state variables.
  
  2. Empirical dynamic modeling (EDM) is an emerging framework for using the mathematical theory of attractor reconstruction to model time series observed from dynamic systems. The `rEDM` package is an implemention of EDM methods in R, building on software developed in the Sugihara Lab.
  ...
  4. `rEDM` facilitates the use of EDM for developing ecological insights through empirical analysis of time series, supporting both exploratory data analysis and confirmation of existing hypotheses."
header-includes: >
  \usepackage{amsmath}
  \usepackage{setspace}
  \doublespacing
...
We review some of the basic concepts underlying EDM, though see @Sauer_1991 for a more detailed treatment. Briefly, time series are sequential observations of a *dynamic system*, which consists of a *state space* and *dynamical rules* for how the system changes over time. At any point in time, the state of the system is a point in the state space (Figure \@ref(fig:time-series-projection)), whose axes are the fundamental state variables. The dynamical rules are typically represented as governing equations for how changes in the system state.
...
When $\theta = 0$, all points receive equal weights, and the local linear map is the same across the reconstruction space, making $G$ equivalent to a single global linear map. For $\theta > 0$, nearby points receive larger weights, and the local linear map depends more on similar system states. Larger values of $\theta$ allow the forecast function, $G$ to be more "wiggly", similar to the scale parameter in a LOESS [REF].
