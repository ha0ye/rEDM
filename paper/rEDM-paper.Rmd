...
In practice, the application of this technique requires selecting an appropriate time lag, $\tau$, embedding dimension, $E$, and methods for inferring $F^\prime$. In the next section, we demonstrate how the **rEDM** software package can be used to accomplish these tasks, with applications for forecasting [@Sugihara_1990], testing for nonlinear behavior [@Sugihara_1994], and testing whether two time series variables belong to the same system and might be causally related [@Sugihara_2012]. 
...
In **rEDM**, the `block_lnlp()` function generalizes the `simplex()` and `s_map()` functions: instead of lags of a single time series, any combination of time series (and their lags) can be used, with either of the Simplex Projection or S-map methods. Now, however, the coordinates of the reconstruction must be manually specified as columns of the input data (`columns`); and similarly for the variable to be forecast (the `target_column` argument). If lags of a time series are desired as coordinates, they need to be pre-computed as separate columns (e.g. via the `make_block()` function).
...
`block_3sp` is a 10-column data-frame, consisting of `time`, and 3 lags for each of the variables: unlagged (`_t-1`), lag-1 (`_t-1`), and lag-2 (`_t-2`). Note that the lagged columns begin with `NA` values because some entries correspond to missing observations of the variables (e.g. `x` at time 0 or -1). Points that contain missing values are automatically excluded from fitting and forecasting (see section [SEC] for more details).

Columns can be referred to by either numerical index (starting with 1) or column name. The optional argument `first_column_time` can be used to indicate that the first column is a time index, and should be ignored when indexing data. For example, supposed that we want to use the coordinates $\left(x_t, x_{t-1}, y_t\right)$ to predict future values of $x_t$. Ignoring the time column, the coordinates of the reconstruction correspond to `columns = c(1, 2, 4)` and `target_column = 1` with `first_column_time = TRUE`.
...
Columns can also be referred to by name. Here, specifying `first_column_time = TRUE` is unnecessary, but will use the values in the time column when labeling predictions.
...
One corollary to Takens' Theorem [-@Takens_1981] is that there are smooth maps between multiple reconstructions of the same system. As an intuitive example, consider a dynamic system that contains two interacting variables, $x$ and $y$. The univariate reconstructions, based on lags of $x$ ( $\mathbf{x}_t = \left( x_t, x_{t-\tau}, \dots, x_{t-(E_x-1)\tau} \right)$) and $y$ ( $\mathbf{y}_t = \left( y_t, y_{t-\tau}, \dots, y_{t-(E_y-1)\tau} \right)$) respectively, can each be used to uniquely identify system states. Thus, there also exists a smooth mapping between $\mathbf{x}_t$ and $\mathbf{y}_t$ (Figure \@ref(fig:cross-mapping)). In practical terms, we test for a mapping between $\mathbf{x}_t$ and $\mathbf{y}_t$ by measuring the predictive skill of mapping from $\mathbf{x}_t$ to $y_t$ and from $\mathbf{y}_t$ to $x_t$.
...
When causation occurs only in one direction, e.g. $x$ influences $y$, but $y$ does not influence $x$, the mapping between $\mathbf{x}_t$ and $\mathbf{y}_t$ is only assured in one direction. Specifically, if $x$ influences $y$, then there is a mapping from $\mathbf{y}_t$ to $x_t$ (i.e. from the affected variable to the causal variable). Although the direction is counterintuitive, this occurs because $\mathbf{y}_t$ must capture all the dynamical rules about how $y$ changes in time, including the influence of $x$. Consequently, $\mathbf{y}_t$ will be able to recover the values of $x$. Conversely, $\mathbf{x}_t$ is only guaranteed to capture the dynamical rules for how $x$ changes in time, which can occur independently of $y$, and so there is no guarantee of a mapping from $\mathbf{x}_t$ to $y_t$.

Nevertheless, because $x$ does influence $y$, there will likely be some predictive skill in the mapping from $\mathbf{x}_t$ to $y_t$. However, this will be limited to a statistical association that will be capped, regardless of the amount of data used to recover the mapping. In contrast, the mapping from $\mathbf{y}_t$ to $x_t$ should continue to improve in skill with more and more data. This convergence is a critical property for inferring causality, and can be tested by measuring the cross mapping skill with larger and larger subsamples of the data. For a more detailed description of using cross mapping to infer causation, see @Sugihara_2012 and @Ye_2015a.
