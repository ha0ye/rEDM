...
In ecology, models are approximations of reality that are used for descriptive, explanatory, or predictive purposes. Conventionally, these models are based on hypothesized parametric equations, formulated from theory, and tested in laboratory or simulation settings. However, there are many situations where explicit mathematical formulations are impractical: the exact mechanisms may be unknown, too complex, or  however explicit equations can be impractical when the exact mechanisms are unknown or too complex to be characterized with existing datasets. 
// many nonlinear interactions, difficult to parameterize using existing datasets, much less perform model selection

// These challenges exacerbated the use of ecological models for predictive purposes: 
effort may be expended on increased monitoring and observation with little improvement. Phenomenological models, which operate by inferring patterns and associations from the data (instead of through hypothesized equations), represent an alternative and highly flexible approach. Here, we review the theoretical background for empirical dynamic modeling (EDM) and the functionality of the `rEDM` package. 

By combining the mathematical theory of attractor reconstruction [@Takens_1981; @Sauer_1991; @Casdagli_1991; @Deyle_2011] and simple methods for function approximation [@Sugihara_1990; @Sugihara_1994], EDM is a powerful approach for practical modeling of complex natural systems using limited time series observations. Because these methods operate with minimal assumptions, they are particularly suitable for studying systems that exhibit non-equilibrium dynamics and nonlinear state-dependent behavior (i.e. where interactions change over time and as a function of the system state).
...
The essential concept underlying EDM is that time series are observations on the behavior of a *dynamic system*. At each point in time, the dynamic system is represented as a point in a high-dimensional state space (Figure \@ref(fig:time-series-projection)). The axes of this space are the fundamental state variables, and the dynamical rules (governing equations) for the system describe how it evolves in time. For example, in an experimental setup with one species of phytoplankton reared in a chemostat, the state variables are population abundance and environmental factors, such as temperature, nutrients, and light; the dynamical rules of the system are the equations for growth and nutrient uptake, and the chemostat settings for temperature, light, and nutrient input.

Sequential observations of plankton abundance comprise a time series of that state variable -- a projection from the high-dimensional state space to that particular coordinate axis. For example, in Figure \@ref(fig:time-series-projection), the states of the canonical Lorenz Attractor [-@Lorenz_1963] are projected to the $x$-axis, creating a time series of variable $x$. In many cases, each time series corresponds to a different state variable, but more generally a time series is an *observation function*, and may be a function of one or more state variables.
...
In order to construct a *conventional* mathematical model of a dynamical system, knowledge of the state variables and the equations, representing the dynamical rules, is required. Uncertainty about the equations or vital parameters can be addressed using model selection and various fitting methods, applied to observational data. However, common challenges in modeling natural systems are structural uncertainty about the state variables or system equations, incomplete or noisy observations, and insufficient data to fully parameterize the equations.

## Attractor Reconstruction / Takens' Theorem

EDM addresses these challenges using the mathematical theory of attractor reconstruction [@Takens_1981; @Sauer_1991; @Casdagli_1991; @Deyle_2011]. The intuition is as follows: because time series are sequential observations of the system behavior, information about the dynamical rules is encoded in the data. Although the conventional wisdom is that knowledge of all the state variables is required; in practice, lags of a time series can substitute for unknown or unobserved variables. For example, instead of using the state variables of $x$, $y$, and $z$ to depict the Lorenz Attractor (as in Figure \@ref(fig:time-series-projection)), we can instead use lags of $x$:
$$ \mathbf{x}_t = \left( x_t, x_{t-\tau}, \dots, x_{t-(E-1)\tau} \right) $$
where $E$ is the embedding dimension, or number of lags.
